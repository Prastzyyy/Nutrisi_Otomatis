...
const char* SUBTOPIC_KALIBRASIPH = "greenhouse/input/kalibrasiph";
const char* SUBTOPIC_KALIBRASIEC = "greenhouse/input/kalibrasiec";
const char* SUBTOPIC_KALIBRASISUHU = "greenhouse/input/kalibrasisuhu";
const char* SUBTOPIC_KALIBRASITINGGI = "greenhouse/input/kalibrasitinggi";
...
const char* PUBTOPIC_KALIBRASIPH = "greenhouse/input/kalibrasiph";
const char* PUBTOPIC_KALIBRASIEC = "greenhouse/input/kalibrasiec";
const char* PUBTOPIC_KALIBRASISUHU = "greenhouse/input/kalibrasisuhu";
const char* PUBTOPIC_KALIBRASITINGGI = "greenhouse/input/kalibrasitinggi";
...
void baca_suhu(){
  ...
  if (suhu != DEVICE_DISCONNECTED_C) {
    // Reduce the temperature value by 0.3
    suhu -= 0.3;
  } else {
...
void callback(char *topic, byte *payload, unsigned int length) {
  ...
  // Kalibrasi pH
  if (!strcmp(topic, SUBTOPIC_KALIBRASIPH)) {
    if (!strncmp(msg, "p1", length)) {
      //nilai += XX;
    } else if (!strncmp(msg, "m1", length)) {
     //nilai -= XX;
    }else if (!strncmp(msg, "p2", length)) {
     //nilai += XX;
    }else if (!strncmp(msg, "m2", length)) {
     //nilai -= XX;
    }
    //client.publish(PUBTOPIC_KALIBRASIPH, String(nilai).c_str());
  }
  // Kalibrasi EC
  if (!strcmp(topic, SUBTOPIC_KALIBRASIEC)) {
    if (!strncmp(msg, "p1", length)) {
      //nilai += XX;
    } else if (!strncmp(msg, "m1", length)) {
     //nilai -= XX;
    ...
    //client.publish(PUBTOPIC_KALIBRASIEC, String(nilai).c_str());
  }
  // Kalibrasi Suhu
  if (!strcmp(topic, SUBTOPIC_KALIBRASISUHU)) {
    if (!strncmp(msg, "p1", length)) {
      //nilai += XX;
    } else if (!strncmp(msg, "m1", length)) {
     //nilai -= XX;
    ...
    //client.publish(PUBTOPIC_KALIBRASISUHU, String(nilai).c_str());
  }
  // Kalibrasi Tinggi
  if (!strcmp(topic, SUBTOPIC_KALIBRASITINGGI)) {
    if (!strncmp(msg, "p1", length)) {
      //nilai += XX;
    } else if (!strncmp(msg, "m1", length)) {
     //nilai -= XX;
    ...
    //client.publish(PUBTOPIC_KALIBRASITINGGI, String(nilai).c_str());
  }
...
void reconnect() {
  while (!client.connected()) {
    ...
    if (client.connect(clientId.c_str(), mqtt_user, mqtt_password)) {
      ...
      client.subscribe(SUBTOPIC_KALIBRASIPH);
      client.subscribe(SUBTOPIC_KALIBRASIEC);
      client.subscribe(SUBTOPIC_KALIBRASISUHU);
      client.subscribe(SUBTOPIC_KALIBRASITINGGI);
